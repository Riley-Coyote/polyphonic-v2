// Incremental SHA-256 so large export archives can be hashed while streaming
// instead of buffering the whole archive text in memory.
...
export class Sha256Stream {
  ...
  updateText(text: string): void {
    this.update(this.encoder.encode(text));
  }
  ...
  hex(): string {
    ...
    const clone = new Sha256Stream();
    clone.h = this.h.slice();
    ...
    let out = "";
    for (let i = 0; i < 8; i += 1) out += clone.h[i].toString(16).padStart(8, "0");
    return out;
  }
